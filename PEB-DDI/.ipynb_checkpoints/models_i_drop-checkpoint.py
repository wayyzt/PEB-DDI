...
class MVN_DDI_Block(nn.Module):
    def __init__(self, in_features, n_heads, head_out_feats, edge_feature, dp):
        ...
        self.feature_conv = GATConv(in_features, head_out_feats, n_heads, edge_dim=edge_feature, dropout=dp)
        self.intraAtt = IntraGraphAttention(head_out_feats*n_heads, dp)
        self.interAtt = InterGraphAttention(head_out_feats*n_heads, dp)
        ...
        self.re_shape_e = Linear(edge_feature, n_heads * head_out_feats, bias=True, weight_initializer='glorot')
        
        # self.drop_block = DropBlock('DropNode')
        
    
    def forward(self, h_data,t_data,b_graph,h_data_edge, t_data_edge):
        # if self.training:
        #     h_data.x, h_data.edge_attr = self.drop_block.drop(h_data.x, h_data.edge_attr, 0.1)
        #     t_data.x, t_data.edge_attr = self.drop_block.drop(t_data.x, t_data.edge_attr, 0.1)
